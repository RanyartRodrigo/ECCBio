...
function coordenadas($coo){
	//$nCoo=str_replace($coo,'','');
	if(strpos($coo,'N')!==false)$nCoo=(floatval(str_replace('N','',$coo)));
        else if(strpos($coo,'S')!==false)$nCoo=(floatval(str_replace('S','',$coo)))*-1;
        else if(strpos($coo,'W')!==false)$nCoo=(floatval(str_replace('W','',$coo)))*-1;
	else if(strpos($coo,'E')!==false)$nCoo=(floatval(str_replace('E','',$coo)));
	return $nCoo;
}
function archivoCSV($nombre){
$nombre_fichero = 'malawi/'.$nombre;
if (file_exists($nombre_fichero)){
$file = fopen("malawi/".$nombre,"r");
$index=0;
$array=array();
$matrix=array(array());
while(! feof($file)){
...
$resultCSV =$obj->consulta("select nombreCSV from encuesta left join encuestaD on encuestaD.idEncuesta=encuesta.idEncuesta  where idGPS=".$_POST['mapaB']." and encuesta.idEncuesta=".$_POST['mapa']." group by nombreCSV");
$numfilasCSV = $resultCSV->num_rows;
for ($x=0;$x<$numfilasCSV;$x++) {
        $fCSV = $resultCSV->fetch_object();
	$flag=1;
	if($fCSV->nombreCSV!=''){
		$matrix=archivoCSV($_POST['mapaB']."-".$fCSV->nombreCSV);
		$tamano=count($matrix);
        	for($i=1;$i<$tamano-1;$i++){
			if($matrix[$i+1][5]!=""){
				if($flag==1 || $i==1){
					$img.="var flightPlanCoordinates = [";
					if($i>1)
						$img.="{lat:".coordenadas($matrix[$i-1][4])." ,lng:".coordenadas($matrix[$i-1][5])."},";
				}
				$img.="{lat:".coordenadas($matrix[$i][4])." ,lng:".coordenadas($matrix[$i][5])."}";
				if($matrix[$i][7]!=$matrix[$i+1][7]){
					$color;
					if($matrix[$i][7]>=0 && $matrix[$i][7]<3)$color="#AA00FF";
                                        if($matrix[$i][7]>=3 && $matrix[$i][7]<7)$color="#CC00FF";
                                        if($matrix[$i][7]>=7 && $matrix[$i][7]<10)$color="#FF00FF";
                                        if($matrix[$i][7]>=10 && $matrix[$i][7]<15)$color="#AAFFFF";
                                        if($matrix[$i][7]>=15)$color="#AAFFFF";
					$img.="];
...
$resultE =$obj->consulta("SELECT * from encuesta where idEncuesta=".$m);
        $encuesta = $resultE->fetch_object();
...
$('#Estad').html(''+
'<p>Date:<br> ".$encuesta->fechaHora."</p><br>'+
'<p>GPS:<br> ".$encuesta->idGPS."</p><br>'+
'<p>CSV:<br> ".$encuesta->nombreCSV."</p><br>'+
'<p>Number:<br> ".$encuesta->numberLast."</p><br>'+
'<p>Number of GPS points in track:<br> ".$encuesta->frequency."</p><br>'+
'<p>Total distance of track:<br> ".$encuesta->sumDistance."</p><br>'+
'<p>Total duration of track:<br> ".$encuesta->sumDuration."</p><br>'+
'<p>Mean speed:<br> ".$encuesta->meanSpeed."</p><br>'+
'<p>Standard desviation of  Speed:<br> ".$encuesta->stdSpeed."</p><br>'+
'<p>Number of GPS points in track Out:<br> ".$encuesta->frequencyOut."</p><br>'+
'<p>Total distance of track Out:<br> ".$encuesta->sumDistanceOut."</p><br>'+
'<p>Total duration of track Out:<br> ".$encuesta->sumDurationOut."</p><br>'+
'<p>Mean Speed Out:<br> ".$encuesta->meanSpeedOut."</p><br>'+
'<p>Standard desviation of  Speed Out:<br> ".$encuesta->stdSpeedOut."</p><br>'+
'');
$('#Estad2').html(''+
'<p>Number of GPS points in track:<br> ".$fila->frequency."</p><br>'+
'<p>Total distance of track:<br> ".$fila->sumDistance."</p><br>'+
'<p>Total duration of track:<br> ".$fila->sumDuration."</p><br>'+
'<p>Mean Speed:<br> ".$fila->meanSpeed."</p><br>'+
'<p>Standard desviation of  Speed:<br> ".$fila->stdSpeed."</p><br>'+
'');
$('#Estad2').removeClass('hiddeEst2');
$('#Estad').removeClass('hiddeEst');

});
                google.maps.event.addListener(polygon, 'mouseout', function (event) {
$('#Estad2').html('');
$('#Estad2').addClass('hiddeEst2');
});
