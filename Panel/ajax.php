...
function ajax_settings_account()
{
	...
	$country   = isset($_POST['id_Pais'])   ? $_POST['id_Pais']   : '';

	$user = User::find(Auth::user()->id);
	$data  = array('country' => $country);
	Usermeta::update($user->id, 'country', $country);
...
function ajax_settings_profile()
{
	...
	if ($validator->passes()) {
		...
		if ($user->save()) {
			...
			$currentAdscriptions = DB::table('adscriptions')->where('user_id', '=', $user->id)->get();
			$toDelete = array();
			$newAdscriptions = array();
			$i=0;
			foreach($currentAdscriptions as $ads)
			{
				$toDelete[$i] = $ads->institution_id;
				$i++;
			}
			if(isset($_POST['adscrip'])){
				...
				foreach($newAdscriptions as $na){
					...
					{ 
						...
						array('user_id' => $user->id, 'institution_id' => $na));
					}
					else
					{
						unset($toDelete[$k]);
...
// Account settings - Image Research
function ajax_researchimage()
{
	if (Auth::guest()) exit;
	$researchid= isset($_GET['researchareaid'])      ?  $_GET['researchareaid'] : "default";
	$options = array(
		'upload_dir' => '../images/researcharea/',
		'upload_url' => '../images/researcharea/',
		'max_file_size' => 5000000, // 5 mb
	    'max_width'  => 2000,
	    'max_height' => 2000,
	    'versions' => array(
		...
		'upload_start' => function($image) use ($researchid) {
			$image->name = "~ra-{$researchid}.{$image->type}";
		},
		'crop_start' => function($image) use ($researchid) {
			$image->name = "ra-{$researchid}.{$image->type}";
		},
		'crop_complete' => function($image) use ($researchid) {
			DB::table('researchareas')
				->where('id', $researchid)
				->update(array('image' => $image->name));
			//Usermeta::update($userId, 'avatar_image', $image->name);
			//Usermeta::update($userId, 'avatar_type', 'image');
		}
	);
...
// Events settings - Image Research
function ajax_eventimage()
{
	if (Auth::guest()) exit;
	$eventid= isset($_GET['eventid'])      ?  $_GET['eventid'] : "default";
	$options = array(
		'upload_dir' => '../images/events/',
		'upload_url' => '../images/events/',
		'max_file_size' => 5000000, // 5 mb
	    'max_width'  => 2000,
	    'max_height' => 2000,
	    'versions' => array(
		...
		'upload_start' => function($image) use ($eventid) {
			$image->name = "~ev-{$eventid}.{$image->type}";
		},
		'crop_start' => function($image) use ($eventid) {
			$image->name = "ev-{$eventid}.{$image->type}";
		},
		'crop_complete' => function($image) use ($eventid) {
			DB::table('events')
				->where('id', $eventid)
				->update(array('image' => $image->name));
			//Usermeta::update($userId, 'avatar_image', $image->name);
...
function ajax_send_message()
{
	...
	if (is_array($message)) {
		...
		if (!empty($email)) {
			...
			if ($user) {
				Mail::send('emails.message', array('body' => $message), function($message) use ($user) {
