...
function createFusionTM(params,offset,limit){
	...
	url.push('sql=');
	var query = 'SELECT '+columns+', '+geometry+',ROWID FROM '+
...
function FusionTableM(params,offset){
	...
	if(this.offset == 0){
		this.ventana = new google.maps.InfoWindow({maxWidth: params[0].maxWidth,zIndex:1001,pixelOffset: new google.maps.Size(0,25)});
		google.maps.event.addListener(this.ventana, "domready", 
			function(){				
				...
				iwCloseBtn.remove();
				//iwCloseBtn.css({opacity: '1',width: '25px',height: '25px',right: '25px',top: '10px','background-color': '#535961',color: 'white','font-size': '20px','font-weight': '700','font-family': 'sans-serif'});
				//iwCloseBtn.html("x");
...
FusionTableM.prototype.addElement = function(element,columns,rowid) {
	var ft = this;
	...
	google.maps.event.addListener(element, 'mouseover', function(e){
		...
		if(lastElement != element){
			...
			/*if(ft.params[1] !==undefined && ft.params[1].infoWindow === undefined && !showOnlyClick){
				if(!ft.areEmpty(columns)){
					var content = ft.constructInfoWindowContent(columns,rowid);
					ft.ventana.setContent(content);
					ft.ventana.setPosition(e.latLng);
					ft.ventana.open(ft.params[0].map);
				}
			}*/
		}
		if(ft.params[1]!==undefined && 
			...
			ft.params[1].caja.mouseover !== undefined)
				ft.params[1].caja.mouseover(e,columns,pol,ft.ventana);
	});
	google.maps.event.addListener(element, 'mouseout', function(e) {
		...
		if(ft.params[1] !==undefined && 
			...
			ft.params[1].caja.mouseout !== undefined)
				ft.params[1].caja.mouseout(e,columns,pol);
	});
	if(ft.params[1]!==undefined && 
		ft.params[1].caja !== undefined && 
			...
			google.maps.event.addListener(element, 'click', function(e) {
				if(ft.params[1] !==undefined && ft.params[1].infoWindow === undefined && showOnlyClick){
					if(!ft.areEmpty(columns)){
						...
						ft.ventana.setPosition(e.latLng);
						ft.ventana.open(ft.params[0].map);
						$('.gm-style-iw').css('background-color','transparent');
						$('.gm-style-iw').css('border','none');
						//$('.gm-style-iw').removeClass('gm-style-iw');
					}
				}
				mapid = ft.params[1].caja.mapid;
				ft.params[1].caja.mouseclick(mapid,rowid);
			});
	}
...
FusionTableM.prototype.constructInfoWindowContent = function(columns,rowid){	
				...
				if( pos == cuerpo[contenido]['pos']){
					...
					while(texto.includes("|")){
						...
						if(cuerpo[contenido]['getExtra'] !== undefined){
							if(number == 0){
								valor = cuerpo[contenido]['getExtra'](rowid);
							}else{
					...
					var onclickText = this.getOnclickText(parametros,onclick,columns,rowid);
...
FusionTableM.prototype.getOnclickText = function(parametros,onclick,columns,rowid){
	var onclickText;
	...
	for(var i=0; i<rows.length; i++) {
		...
		if(typeFigure == "Polygon"){
			...
			if(newCoordinates.length >= 2){
				coords = newCoordinates[0][0];
			} else {
				coords = newCoordinates[0];
			}			
		...
		if(element != null){
			this.addElement(element,columns,rows[i][tamCols+1]);
		}
	}
	if(this.params[0].storePol !== undefined){
