<?php
	...
	$imagesP = $_REQUEST['imgProt'];
	$imagesF = $_REQUEST['imgsFrag'];
	$imagesT = $_REQUEST['imgsTend'];
	$imagesE = $_REQUEST['imgsEst'];
	$imagesMann = $_REQUEST['imgsMann'];
	...
	$captionProt = $_REQUEST['captionProt'];
	$tablaStadistics = $_REQUEST['tablaStadistics'];
	$tituloNombre = $_REQUEST['titulosCompletos'];
	$tituloCategoria = $_REQUEST['titulosCompletosCategoria'];
	$protConnReporte = $_REQUEST['protConnReporte'];
	$protConnEcoregiones = $_REQUEST['protConnEcoregiones'];

	//echo($tablaStadistics);
	//echo($titulos);
	//die();
	...
	for($i = 0; $i < $n; $i++){
		...
		//Crear las imagenes
	 	...
	 	file_put_contents("LaTeX/{$name}dir/{$id}F.png", file_get_contents($imagesF[$i]));
	 	file_put_contents("LaTeX/{$name}dir/{$id}T.png", file_get_contents($imagesT[$i]));
	 	file_put_contents("LaTeX/{$name}dir/{$id}E.png", file_get_contents($imagesE[$i]));
	 	file_put_contents("LaTeX/{$name}dir/{$id}Mann.png", file_get_contents($imagesMann[$i]));

	 	//Crear los titulos
	 	$titEspecial = html_entity_decode($titulos[$i]);
	 	$titEspecial1 = html_entity_decode($tituloNombre[$i]);
	 	//$titEspecial = "RB Sian Ka'an";
	 	//$titEspecial1 = "Sian Ka'an";

	 	$titEspecial2 = html_entity_decode($tituloCategoria[$i]);
	 	...
	 	exec("touch LaTeX/{$name}dir/tituloNombre{$id}.txt && echo $titEspecial1 > LaTeX/{$name}dir/tituloNombre{$id}.txt");
	 	exec("touch LaTeX/{$name}dir/tituloCategoria{$id}.txt && echo $titEspecial2 > LaTeX/{$name}dir/tituloCategoria{$id}.txt");
		...
		//caption
		exec("touch LaTeX/{$name}dir/captionProt{$id}.txt");
		$arrayInfo = $captionProt[$i];
		for ($l = 0; $l < 2; $l++){
			exec("echo {$arrayInfo[$l]} >> LaTeX/{$name}dir/captionProt{$id}.txt");			
		}

		//statics
		exec("touch LaTeX/{$name}dir/statics{$id}.txt");
		$arrayInfo = $tablaStadistics[$i];
		for ($l = 0; $l < 25; $l++){
			exec("echo {$arrayInfo[$l]} >> LaTeX/{$name}dir/statics{$id}.txt");			
		}

		//protConnTable data
		exec("touch LaTeX/{$name}dir/protConnTable{$id}.txt");
		$arrayInfo = $protConnReporte[$i];
		for ($l = 0; $l < 12; $l++){
			exec("echo {$arrayInfo[$l]} >> LaTeX/{$name}dir/protConnTable{$id}.txt");
		}

		//protConnEcoregiones
		exec("touch LaTeX/{$name}dir/protConnEcoregiones{$id}.txt");
		$arrayInfo = $protConnEcoregiones[$i];
		for ($l = 0; $l < 3; $l++){
			$ecoRegion = html_entity_decode($arrayInfo[$l]);
			exec("echo {$ecoRegion} >> LaTeX/{$name}dir/protConnEcoregiones{$id}.txt");
		}
	...
	exec("cp LaTeX/templates/template.tex LaTeX/{$name}dir/");
	...
	exec("cp LaTeX/templates/logos/*.jpg LaTeX/{$name}dir/logos/");
	...
	$comandoRubber2 = "cd LaTeX/{$name}dir; /usr/local/texlive/2018/bin/x86_64-linux/xelatex --shell-escape -interaction=nonstopmode template.tex && /usr/local/texlive/2018/bin/x86_64-linux/bibtex template.aux && /usr/local/texlive/2018/bin/x86_64-linux/xelatex --shell-escape -interaction=nonstopmode template.tex && /usr/local/texlive/2018/bin/x86_64-linux/xelatex --shell-escape -interaction=nonstopmode template.tex && mv template.pdf $name2.pdf";
	...
	$ret = "http://www.wegp.unam.mx/assets_conabio2/reportesPDF/LaTeX/{$name}dir/$name2.pdf";
